    ...
    <el-dialog title="分配角色" :visible.sync="dialogFormVisibleRol">
      <el-form :model="form">
        ...
        <el-form-item label="角色" label-width="100px">
          <!--如果select的绑定的数据的值 和option的value一样 就会显示该option的label值-->
          {{currRoleId}}
  ...
  methods: {
    ...
    async changeMgState (user) {
      ...
      await this.$http.put(
        // 1.mg_state =false
        // 2. 点开关 -> mg_state=true
        `users/${user.id}/state/${user.mg_state}?rid=${user.role_id}`)
    },
    // 编辑用户 -发送请求
    async editUser () {
   ...
   --> */
    ...
    async getUserList () {
      ...
      const {meta: {status}, data, total} = res.data
      if (status === 200) {
        // 1. 给表格数据赋值
        for (const key in data) {
          data[key].mg_state = Boolean(data[key].mg_state)
        }
        this.userlist = data
        // 2. 给total赋值 console.log(`每页 ${val} 条`)
        this.total = total
        // 3. 提示
      } else {
        // this.$message.warning(msg)
